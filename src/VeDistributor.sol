...
/// @author belbix
contract VeDistributor is Controllable, IVeDistributor {
    ...
    function init(address controller_, address _ve, address _rewardToken) external initializer {
        ...
        IERC20(_rewardToken).approve(_ve, type(uint).max);
    }
    ...
    function adjustToDistribute(uint toDistribute, uint t0, uint t1, uint sinceLast) public pure returns (uint) {
    ...
    function veForAt(uint _tokenId, uint _timestamp) external view returns (uint) {
        ...
        return uint(int(_positiveInt128(pt.bias - pt.slope * (int128(int(_timestamp - pt.ts))))));
    }
    ...
    function adjustVeSupply(uint t, uint ptTs, int128 ptBias, int128 ptSlope) public pure returns (uint) {
        ...
        int128 dt = int128(int(t - ptTs));
        if (ptBias < ptSlope * dt) {
            return 0;
        }
        return uint(int(_positiveInt128(ptBias - ptSlope * dt)));
    }
    ...
    function _calculateClaim(
        ...
        return calculateToDistribute(_tokenId, weekCursor, _lastTokenTime, userPoint, userEpoch, maxUserEpoch, _ve);
    }
    ...
    function calculateToDistribute(
        ...
        for (uint i = 0; i < 50; i++) {
            ...
            if (weekCursor >= userPoint.ts && userEpoch <= maxUserEpoch) {
                ...
                int128 dt = int128(int(weekCursor - oldUserPoint.ts));
                uint balanceOf = uint(int(_positiveInt128(oldUserPoint.bias - dt * oldUserPoint.slope)));
        ...
        return
            ClaimCalculationResult(toDistribute, Math.min(maxUserEpoch, userEpoch - 1), weekCursor, maxUserEpoch, true);
    }
