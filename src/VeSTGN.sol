...
contract VeSTGN is Controllable, ReentrancyGuard, IVe {
  ...
  string constant public override name = "veSTGN";
  string constant public override symbol = "veSTGN";
  ...
  function _depositFor(DepositInfo memory info) internal {
    ...
    emit Deposit(info.stakingToken, msg.sender, info.tokenId, info.value, newLockedEnd, info.depositType, block.timestamp);
  }
  ...
  function _balanceOfAtNFT(uint _tokenId, uint _block) internal view returns (uint) {
    ...
    return VeSTGNLib.balanceOfAtNFT(
      _tokenId,
      _block,
      epoch,
      userPointEpoch,
  ...
  function totalSupplyAt(uint _block) external view override returns (uint) {
    return VeSTGNLib.totalSupplyAt(
      _block,
      epoch,
      _pointHistory,
      slopeChanges
